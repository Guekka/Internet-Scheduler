...
// From https://github.com/Baduit/Timer/blob/master/src/Timer.hpp
...
public:
    ...
    {
        return std::chrono::duration_cast<T>(get_time_duration());
    }
...
public:
    ...
    auto get_time_duration() { return _run_timer.get_time_duration() - get_total_pause_time(); }
    ...
    {
        return (_pause_timer.has_value()) ? _paused_time + _pause_timer->get_time_duration()
                                          : _paused_time;
    }

    template<typename T>
    auto get_total_pause_time_as()
    {
        return std::chrono::duration_cast<T>(get_total_pause_time());
    }

    int64_t get_total_pause_time_nano_count()
    {
...
{
public:
    enum class LoopStatus { CONTINUE, STOP };
...
enum class ActionOnDtor { JOIN, STOP };
...
public:
    ...
    virtual ~ThreadTimer() { stop(); }

    template<typename D, typename Cb>
    auto start(const D &duration, Cb &&cb)
    {
        auto thread_cb = [this, duration, moved_cb = std::forward<Cb>(cb)]() {
            _sleeper(duration);
            if (!_stop)
                return moved_cb();
            else
        ...
        auto task = std::packaged_task<decltype(thread_cb())()>(
            std::forward<decltype(thread_cb)>(thread_cb));
